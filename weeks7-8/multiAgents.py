...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def maxFunction(gameState,depth):
            ...
            scores = [minFunction(gameState.generateSuccessor(self.index,move),1, depth) for move in moves]
            bestScore=max(scores)
            bestIndices = [index for index in range(len(scores)) if scores[index] == bestScore]
        ...
        def minFunction(self,gameState,agent, depth):  
            ...
            if(agent!=gameState.getNumAgents()-1):
              scores =[minFunction(gameState.generateSuccessor(agent,move),agent+1,depth) for move in moves]
            else:
              scores =[maxFunction(gameState.generateSuccessor(agent,move),(depth-1))[0] for move in moves]
            minScore=min(scores)
        ...
        bestScore,bestMove=maxFunction(gameState,self.depth)
        return bestMove
